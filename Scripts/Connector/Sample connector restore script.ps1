...
# 2) Install the latest Connector from https://aka.ms/UPConnector (do not register it)
 
# Stop services
net stop "Print Connector service"; net stop PrintConnectorUpdaterSvc
...
New-Item -Path "CERT:\LocalMachine\PrintProxyStore"; $pwd = ConvertTo-SecureString -String "password" -AsPlainText -Force; Get-ChildItem "C:\ConnectorBackup\Certs" | Foreach-Object { Import-PfxCertificate -Password $pwd -FilePath $_.FullName -CertStoreLocation Cert:\localmachine\PrintProxyStore -Exportable }
 
# Import conector registries
...
# ENSURE THE CONNECTOR SERVICE HAS BEEN STOPPED IN THE OLD MACHINE BEFORE RUNNING THIS LAST COMMAND

# Start services
net start "Print Connector service"; net start PrintConnectorUpdaterSvc
